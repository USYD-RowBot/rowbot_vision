...
#    amt_to_print=1;
    ...
    for id in IDs: # print out top 5 only
        if id['color'] == "u":
            ...
            c *= ratio
            c = c.astype("int")
            cv2.drawContours(blank_image, [c], -1, (20, 20, 20), 2)
            cv2.putText(blank_image, id['color'] + id['shape'], (cX, cY), cv2.FONT_HERSHEY_SIMPLEX,
                0.5, (20, 20, 20), 2)
            result=id['color'] + id['shape']
            #amt_to_print-=1;
            #if amt_to_print==0: break;
    return (blank_image,result);
