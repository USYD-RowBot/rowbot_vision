...
void BuoyDetector::classifyBuoys(cv::Mat image, bool printToScreen) {
    if (image.data) {
        ...
        m_objects = classifier.m_objects;
    } else {
...
Object* BuoyDetector::getBestAtBearing(float bearing, int objsInFront)  {
    if (m_objects.size() == 0) {
        std::cout << "No Objects "  <<std::endl;
        return NULL;
    }
    float margin = 0.5*(float)m_cameraFOV; // 10% of camera FOV
    float highestConfidence = -1;
    int index = -1;
    for (int i = 0; i < m_objects.size(); ++i) {
      std::cout << margin << bearing << std::endl;
