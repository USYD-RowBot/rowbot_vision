...
class ShapeDetector:
    ...
    def identify(self,img,filters,bearing=None):
        ...
        cnts = filters.getContours(img, ['notcolor'], True)
        nextCnt = []
        rectIndices = []
        # get outer level contorus.
        for c, cnt in enumerate(cnts['notcolor'][0]):
            # check if they are rectangles
            # will not work because the contours will be hollow!
            r = cv2.boundingRect(cnt)
            nextCnt.append((cnt, r, cnts['notcolor'][1][c]))
            # append outermost white rectangles
            if (cnts['notcolor'][1][c][3] == -1):
        ...
        for cnt in cnts:
            ...
            # ,['s_hsv'])# implement second argument to save time for later
                ...
                col= np.sum(img2)/cv2.contourArea(cnt)
                color='derp'
                if 0<col<30:
                    col='red'
                elif 45<col<95:
                    ...
                    col='blue'
                # check the shape
                if shape != "weird" and col!='derp' and cA > 80 and rat > 0.7 and rat < 1.5:
                    IDs.append({'area': cA, 'cnt': c, 'shape': shape,
                                'color': col, 'confidence': cA})
                    if self.debug_level>50:
                        cv2.drawContours(dbg_img, [c], -1, [0, 255, 0], 4)
                        cv2.putText(dbg_img, col+" "+shape, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 2, [0, 255, 0])
